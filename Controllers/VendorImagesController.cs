...
namespace FoodHub.Controllers
{
    public class VendorImagesController : Controller
    {
        ...
        {
            return View(db.VendorImages.ToList());
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,FileName")] VendorImage vendorImage)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            VendorImage vendorImage = db.VendorImages.Find(id);
            var mappings = vendorImage.VendorImageMappings.Where(pim => pim.VendorImageId == id);
            foreach (var mapping in mappings)
            {
                //find all mappings for any product containing this image
                var mappingsToUpdate = db.VendorImageMappings.Where(pim => pim.VendorId == mapping.VendorId);
            ...
            System.IO.File.Delete(Request.MapPath(Constants.FoodImagePath + vendorImage.FileName));
            System.IO.File.Delete(Request.MapPath(Constants.FoodThumbnailPath + vendorImage.FileName));
        ...
        private void SaveFileToDisk(HttpPostedFileBase file)
        {
            ...
            img.Save(Constants.VendorImagePath + file.FileName);
            if (img.Width > 100)
            {
                img.Resize(100, img.Height);
            }
            img.Save(Constants.VendorThumbnailPath + file.FileName);
        }
    }
}
